...
//  Msg-AlertController
//
//  Created by John Hur on 2021/07/20.
//
...
class ViewController: UIViewController {
    ...
    @IBAction func alert(_ sender: Any) {
        ...
        let cancel = UIAlertAction(title: "취소", style: .cancel) {
            (_) in self.result.text = "취소 버튼을 클릭했습니다" } // 취소버튼
        let ok = UIAlertAction(title: "확인", style: .default) {
            (_) in self.result.text = "확인 버튼을 클릭했습니다" } // 확인 버튼
        let exec = UIAlertAction(
            title: "실행", style: .destructive) { (_) in self.result.text = "실행 버튼을 클릭했습니다" } // 실행 버튼
        let stop = UIAlertAction(title: "중지", style: .default) { (_) in self.result.text = "중지 버튼을 클릭했습니다" } // 중지 버튼
